...
async def execute_lxc(container_name: str, command: str, timeout=120, node_id: Optional[int] = None):
    ...
    # Keep the existing call surface intact while translating the old
    # container operations to Docker operations.  This avoids changing any
    # Discord/database behaviour outside the container runtime.
    ...
    if operation == "list":
        ...
        if operation == "delete":
            action = "rm -f"
        docker_command = f"docker {action} {shlex.quote(parts[1])}"
    elif operation == "delete":
        docker_command = f"docker rm -f {shlex.quote(parts[1])}"
    elif operation == "exec":
        target = parts[1]
        exec_args = parts[3:] if len(parts) > 2 and parts[2] == "--" else parts[2:]
        docker_command = f"docker exec {shlex.quote(target)} " + " ".join(shlex.quote(p) for p in exec_args)
        ...
        if len(parts) >= 5 and parts[1] == "set" and parts[3] == "limits.memory":
            ...
            # container is already isolated on its Docker network.
            docker_command = "true"
        else:
            docker_command = "true"
    elif operation == "copy":
        source, target = parts[1], parts[2]
        docker_command = f"docker commit {shlex.quote(source)} {shlex.quote(target)}"
    elif operation == "rename":
        docker_command = f"docker rename {shlex.quote(parts[1])} {shlex.quote(parts[2])}"
    elif operation == "snapshot":
        if len(parts) >= 3 and parts[1] == "list":
            docker_command = "docker images --format '{{.Repository}}:{{.Tag}}\t{{.CreatedSince}}' "
        else:
            docker_command = f"docker commit {shlex.quote(parts[1])} {shlex.quote(parts[2])}"
    elif operation == "restore":
        # Restore is handled as an image replacement; the caller starts it.
        docker_command = (
            f"docker rm -f {shlex.quote(parts[1])} && "
            f"docker create --name {shlex.quote(parts[1])} --privileged "
            f"{shlex.quote(parts[2])} /bin/sh -c 'while true; do sleep 3600; done'"
        )
    elif operation in ("storage", "profile"):
        docker_command = "docker info"
    else:
        # Commands not represented by the small compatibility layer are
        # executed as Docker commands, rather than as LXC commands.
        docker_command = "docker " + command
    full_command = docker_command
    ...
    if node['is_local']:
        ...
        try:
            ...
            logger.error(f"Remote Docker error on node {node['name']} ({url}): {str(e)}")
...
# Apply Docker runtime config
async def apply_lxc_config(container_name: str, node_id: int):
    try:
        await execute_lxc(container_name, f"update --restart unless-stopped {container_name}", node_id=node_id)
        logger.info(f"Docker permissions applied to {container_name} on node {node_id}")
    except Exception as e:
        logger.error(f"Failed to apply Docker config to {container_name}: {e}")
...
async def get_container_status_local(container_name: str):
    try:
        ...
        return stdout.decode().strip().lower() if stdout else "unknown"
    except Exception:
        return "unknown"
...
@bot.command(name='docker-list', aliases=['lxc-list'])
@is_admin()
            ...
            try:
                ...
                for node in nodes:
                    if node['is_local']:
                        ...
                        url = f"{node['url']}/api/execute"
                        data = {"command": "docker ps -q | xargs -r docker stop"}
                        params = {"api_key": node["api_key"]}
                        response = requests.post(url, json=data, params=params)
                        if response.status_code != 200:
